...
import {
  fetchLatestSubmissionsCodeForces,
...
const QuestionSolving = () => {
  ...
  const [codeforcesApiCredentials, setCodeforcesApiCredentials] = useState<{
    apiKey: string | null;
  ...
  const verifySubmission = async (
    ...
    try {
      if (platform === "Leetcode") {
        ...
        return (
          res?.recentSubmissionList?.some(
            (p: LeetCodeSubmission) =>
              p.titleSlug === problemName && p.statusDisplay === "Accepted"
          ) || false
        );
      } else {
        // Use user's API credentials if available
        const res = await fetchLatestSubmissionsCodeForces(
          username,
          codeforcesApiCredentials
        );
        return (
          res?.some(
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setQuestions((prev) =>
          prev.map((q) =>
            q.id === questionId
              ? { ...q, isBookmarked: !currentBookmarkStatus }
              : q
          )
        );

        toast.success(
          currentBookmarkStatus ? "Bookmark removed" : "Question bookmarked"
        );
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (
          pUsernames.codeforcesUsername === "" ||
          pUsernames.leetcodeUsername === ""
        ) {
        ...
        const questionsRes = await axios.post(
          "/api/questions",
          newTopics && difficulties
            ? { topics: [...newTopics], difficulties: [...difficulties] }
            : {}
        );
        questionsRes.data.questionsWithSolvedStatus.forEach((q: Question) => {
          setLocalTags((prev) => {
            return [
              ...new Set([
                ...prev,
                ...q.questionTags.map((tags: QuestionTag) => tags.name),
              ]),
            ];
          });
        });
  ...
  useEffect(() => {
    ...
    if (selectSolved) {
      filtered = filtered.filter(
        (q) => !q.isSolved && !solvedProblems.has(q.id)
      );
    }
    ...
    if (selectedTags.length > 0) {
      filtered = filtered.filter((q) => {
        ...
        return selectedTags.every((selectedTag) =>
          questionTagNames.includes(selectedTag)
        );
      });
    }
            ...
            <CardContent className="py-6 border-none">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="text-right">
                  ...
                  />
                  <Skeleton
                    className={`h-8 w-16 ${isDarkMode ? "bg-[#262626]" : ""}`}
                  />
                </div>
              </div>
              <div className="space-y-4">
                <div className="flex items-center gap-4">
                  <Skeleton
                    className={`h-4 w-32 ${isDarkMode ? "bg-[#262626]" : ""}`}
                  />
                  <Skeleton
                    className={`h-10 w-48 ${isDarkMode ? "bg-[#262626]" : ""}`}
                  />
                </div>
                <div className="space-y-2">
                  <Skeleton
                    className={`h-4 w-24 ${isDarkMode ? "bg-[#262626]" : ""}`}
                  />
              ...
              >
                <CardHeader>
                  <div className="flex items-center justify-between">
                    <Skeleton
                      className={`h-6 w-48 ${isDarkMode ? "bg-[#262626]" : ""}`}
                    />
                    <Skeleton
                      className={`h-6 w-20 ${isDarkMode ? "bg-[#262626]" : ""}`}
                    />
                  </div>
                ...
                <CardContent>
                  <div className="flex items-center justify-between">
                    ...
                    />
                    <Skeleton
                      className={`h-9 w-24 ${isDarkMode ? "bg-[#262626]" : ""}`}
                    />
                  </div>
                </CardContent>
              </Card>
  ...
  return (
    ...
    >
      <div
        className={`container mx-auto px-4 py-8 md:py-30 pt-20 space-y-8 max-w-6xl`}
      >
          ...
          <CardHeader className="py-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                >
                  Master algorithms through consistent practice
                </p>
              </div>
              <div
                className={`${
                  isDarkMode
                    ? "bg-[#262626] border-none"
                    : "bg-white border-none"
                } p-4 rounded-lg shadow-none border`}
              >
                <p
                  className={`text-sm ${
            ...
            <div className="space-y-6">
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center gap-4 mb-4">
                  ...
                  >
                    <SelectTrigger
                      className={`border-none w-full md:w-48 ${
                        isDarkMode
                          ? "bg-[#484848]  text-gray-200"
                    ...
                    >
                      {DIFFICULTIES.map((difficulty) => (
                        <SelectItem
                          ...
                          className={
                            isDarkMode ? "text-gray-200 hover:bg-[#484848]" : ""
                          }
                        >
                          {difficulty.label}
                        </SelectItem>
                  ...
                  <Button
                    className={`${
                      isDarkMode
                        ? "bg-[#484848] hover:bg-zinc-700 text-white"
                        : "bg-black hover:bg-zinc-800 text-white"
                    }`}
                    onClick={() => setSelectSolved((p) => !p)}
                  >
                    Toggle not Solved
                  </Button>
                </div>
        ...
        {filteredQuestions.length === 0 ? (
          ...
          >
            ...
            <Button
              ...
              className={`mt-4 ${
                isDarkMode
                  ? "border-gray-500 text-gray-300 hover:bg-[#262626]"
                  : "border-indigo-200 text-indigo-700 hover:bg-indigo-50"
              }`}
              onClick={() => {
                setSelectedTags([]);
          ...
          <div className="grid gap-6">
            {filteredQuestions.map((q) => {
              ...
              return (
                <Card
                  key={q.id}
                  className={`border-none transition-all duration-300 hover:shadow-none ${
                    q.isSolved || solvedProblems.has(q.id)
                      ? `${
                          isDarkMode
                            ? "bg-[#262626] border-zinc-700/50"
                            : "bg-zinc-100/50 border-zinc-200"
                        }`
                      : `${
                          isDarkMode
                            ? "bg-[#262626] border-gray-700 hover:border-indigo-500/50"
                    ...
                    <div className="flex flex-wrap gap-2 mt-3">
                      {q.questionTags.map((tag: AnyTag) => (
                        <Badge
                          ...
                          className={`text-xs ${
                            isDarkMode
                              ? "bg-[#262626] border-gray-600 text-gray-300"
                        ...
                        <Button
                          ...
                          className={`${
                            ...
                            q.isBookmarked
                              ...
                              : isDarkMode
                              ? "text-indigo-400 hover:bg-[#262626]"
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            const response = await verifySubmission(
                              q.leetcodeUrl ? "Leetcode" : "Codeforces",
                              q.slug,
                              q.leetcodeUrl
                                ? pUsernames.leetcodeUsername
                                : pUsernames.codeforcesUsername,
                          ...
                          className={`${
                            isDarkMode
                              ? "border-gray-600 text-indigo-400 hover:bg-[#262626]"
                              : "border-indigo-200 text-indigo-700 hover:bg-indigo-50"
                          } w-full sm:w-auto`}
                        >
                          {isVerifying[q.id] ? (
        ...
        {questions.length > 0 && (
          ...
          >
            ...
            {showBookmarkedOnly && (
              <span className="ml-1">(Bookmarked only)</span>
            )}
          </div>
        )}
      </div>
