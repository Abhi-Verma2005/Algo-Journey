...
import { validateExternalProfiles } from "@/serverActions/fetch";
...
const signupSchema = z.object({
  ...
  email: z
    ...
    .refine((email) => {
      ...
      return (
        ...
        lower.endsWith("@newtonschool.co") ||
        lower.endsWith("@sst.scaler.com") ||
        lower.endsWith("@adypu.edu.in") ||
        lower.endsWith("@gla.ac.in") ||
        lower.endsWith("@oriental.ac.in") ||
        lower.endsWith("@maimt.com") ||
        lower.endsWith("@csds.rishihood.edu.in") ||
        lower.endsWith("@svyasa-sas.edu.in")
      );
    }, "Must use your official college email"),
  ...
  enrollmentNum: z.string().trim().min(1, "Enrollment number is required"),
...
export default function Signup() {
  ...
  const onSubmit = async (data: SignupFormData) => {
    ...
    try {
      // 1. Validate platform username format (non-blocking for rate limits)
      const platformValidation = await validateExternalProfiles(
        data.leetcodeUsername,
        data.codeforcesUsername,
      );
      if (!platformValidation.valid) {
        toast.error(platformValidation.message || "Invalid platform username");
        setIsSubmitting(false);
        return;
      }

      // 2. Submit user registration and initial sync
      ...
      toast.success("Signup successful! Please sign in with your credentials.");
          ...
          <div className="max-w-3xl">
            <svg viewBox="0 0 900 700" className="w-full h-auto opacity-90">
              ...
              <g className="opacity-85">
                <text
                  x="650"
                  y="130"
                  className="fill-cyan-300 text-lg font-bold"
                >
                  Stack
                </text>
                {[0, 1, 2, 3, 4].map((i) => (
              ...
              <g className="opacity-80">
                <text
                  x="100"
                  y="450"
                  className="fill-emerald-300 text-lg font-bold"
                >
                  Merge Sort
                </text>
              ...
              <g className="opacity-75">
                <text
                  x="450"
                  y="450"
                  className="fill-teal-300 text-lg font-bold"
                >
                  Hash Table
                </text>
                {[0, 1, 2, 3, 4].map((i) => (
                  <g key={i}>
                    ...
                    <text
                      x="500"
                      y={487 + i * 30}
                      className="fill-cyan-200 text-xs"
                    >
                      {i === 0
                        ? "key1 → value1"
                        : i === 2
              ...
              <text
                x="50"
                y="80"
                className="fill-emerald-300 text-2xl font-mono opacity-70"
              >
                O(n log n)
              </text>
              <text
                x="700"
                y="80"
                className="fill-cyan-300 text-2xl font-mono opacity-70"
              >
                O(1)
              </text>
              <text
                x="350"
                y="380"
                className="fill-teal-300 text-xl font-mono opacity-70"
              >
                O(V + E)
              </text>
              ...
              <text
                x="50"
                y="350"
                className="fill-yellow-300 text-sm font-mono opacity-60"
              >
                while(left &lt; right)
              </text>
              <text
                x="600"
                y="350"
                className="fill-orange-300 text-sm font-mono opacity-60"
              >
                return dp[n]
              </text>
              ...
              <text
                x="80"
                y="200"
                className="fill-emerald-300 text-8xl font-mono opacity-30"
              >
                {"{"}
              </text>
              <text
                x="750"
                y="500"
                className="fill-cyan-300 text-8xl font-mono opacity-30"
              >
                {"}"}
              </text>
            </svg>
            ...
            <div className="text-center mt-10">
              <h1 className="text-4xl lg:text-5xl font-bold text-white mb-4">
                Join the Journey
                <span className="block text-3xl lg:text-4xl text-emerald-300 mt-2">
                  to DSA Mastery
                </span>
              </h1>
              <p className="text-white/70 text-lg max-w-lg mx-auto leading-relaxed">
                Connect with peers, track your progress, and master algorithms
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            placeholder="your.name@nst.rishihood.edu.in"
                            className={`h-11 border-gray-200 ${emailParam ? "bg-gray-50" : "focus:border-emerald-500 focus:ring-emerald-500"}`}
                            {...field}
